...
const pastOrderReducer = (state, action) => {
  console.log(action);
  console.log(state);
...
export default function AllOrder() {
  ...
  const [state, dispatch] = React.useReducer(pastOrderReducer, initialState);

  React.useEffect(() => {
    async function fetchData() {
      const res = await get("/api/order/pastOrder");
  ...
  const handleNextPage = async () => {
    if (state.isLastPage === false) {
      const nextPage = await get("/api/order/pastOrder", {
  ...
  const handlePrevPage = async () => {
    if (state.isFirstPage === false) {
      const prevPage = await get("/api/order/pastOrder", {
        page: state.pageNum - 1
      });
        ...
        <GridItem xs={12} sm={12} md={12}>
          ...
          >
            <Table className={classes.table} aria-label="simple table">
              <TableHead>
                <TableRow>
                  <TableCell align="left">订单号码</TableCell>
